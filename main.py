...
def main():
    ...
    context = """The Amazon rainforest (Portuguese: Floresta Amazônica or 
    Amazônia; Spanish: Selva Amazónica, Amazonía or usually Amazonia; French: 
    ...
    Colombia with 10%, and with minor amounts in Venezuela, Ecuador, Bolivia, Guyana, 
    Suriname and French Guiana. States or departments in four nations contain "Amazonas" 
    in their names. The Amazon represents over half of the planet's remaining rainforests,
    ...
    bert_model = bm.prepare_model()
    x_data, input_ids = dp.data_preprocessing(context, question, tokenizer)
    answer_start, answer_stop = bert_model.predict(x=x_data)

    decoded_tokens = tokenizer.convert_ids_to_tokens(input_ids)

    print(decoded_tokens[np.argmax(answer_start):np.argmax(answer_stop)])
